...
<h3>Easy solution?</h3>
      ...
      gap: "60px" // space between canvases
        ...
        <section>
          <h3>\(01\)-Sequences</h3>
          <p>For \(\lambda\), imagine putting \(0\)s and \(1\)s along the border with \(0\) if it is a vertical segment and \(1\) otherwise. This gives the \(01\)-sequence \(s_\lambda\).</p>
          <canvas ref={canvases[17]} width={330} height={240} />
        </section>

        <section>
          <h3>\(01\)-Sequences</h3>
          <p> We want to index it! The line \(y=-x\) seperates the negative indices from the non-negatives; read off as normal.</p>
        ...
        <section>
          ...
          <p>Can induct on \(t\) with base case \(t=1\). If segment is {String.raw`\(1\underbrace{1\cdots 1}_{t-1}0\)`}, result is easy to see; otherwise can find \(a\) with {String.raw`\(s_{i+a}=0\)`}, then {String.raw`\((s_i,s_{i+a},s_{i+t})=(1,0,0)\)`}, do \((1,0,0) \mapsto (0,1,0) \mapsto (0,0,1)\). Removing an \(a\)-rim hook and then a \(t-a\)-rim hook starting at the same index is same as removing \(t\)-rim hook. </p>
        </section>
        ...
        <section>
          ...
          <p>Due to how indexing is defined, \(\sum n_i = 0\).</p>
        ...
        <section>
          ...
          <p>To see that it's the inverse of the previous algorithm, only really need to check that shifting these sequences gives us the right core. However, removing all \(t\)-rim hooks converts \(s^i\) into \(\bar{0}\bar{1}\), and then this is just the bijection between cores and vectors.</p>
        </section>
        ...
        <section>
          ...
          <ul>
            <li>Assign each group element \(g\) an \(n \times n\) matrix \(\rho_g\) such that {String.raw`\(\rho_{gh}=\rho_g\rho_h\)`}. Then the \(\rho_g\) are a <strong>representation</strong> of \(G\).</li>
            <li>If {String.raw`\(W\subseteq \mathbb{C}^n\)`} has \(\rho_gW \subseteq W\) for all \(g\), then can write \(W \oplus V \cong \CC^n\) and find representations \(\phi_g,\psi_g\) so that \(\rho_g = \phi_g \oplus \psi_g\). </li>
            <li>For any finite group, can continue until the \(W\) are as small as possible. These correspond to <strong>irreducible representations</strong>, finite groups only have finitely many.</li>
        ...
        <section>
          ...
          <p>For each \(\lambda\), can construct irreducible reps \(\rho_\lambda\)!</p>
        ...
        <section>
          ...
          <p>Over {String.raw`\(\mathbb{C}\)`}, representations factor as direct sum of irreducibles; Over \(K\), indecomposables have chains of subrepresentations whose quotients are irreducibles...</p>
        ...
        <section>
          <h3>Back to \(S_n\)</h3>
          <p>(Nakayama's Conjecture (proven)) Let \(\lambda, \mu \vdash n\), then \(\rho_\lambda, \rho_mu\) belong to the same block if and only if they have the same \(p-\)core. </p>
